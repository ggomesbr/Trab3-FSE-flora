...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_SUBSCRIBED:
        ...
        msg_id = esp_mqtt_client_publish(client, "/topic/qos0", "data", 0, 0, 0); //aqui eu tenho que dar subscribe no canal que a esp_bateria vai enviar as info
...
void comunicacao_servidor_task(void *params)
{
    ...
    if (xSemaphoreTake(conexaoMQTTSemaphore, portMAX_DELAY))
    {
        while (true)
        {
            mqtt_envia_mensagem("v1/devices/me/telemetry", "teste 1");

            mqtt_envia_mensagem("v1/devices/me/attributes", "teste 2");
